...
```{r-ModelFitting}
start <- Sys.time()
model_lda_22 <- LDA(dfm_matrix_tm, 
                    k = 22, method = 'Gibbs', 
                    control = list(alpha = 0.08, burnin = 500 , verbose = 100 , seed = 1234))
stop <- Sys.time()

print(stop-start)
saveRDS(model_lda_22,'./Models/model_lda_22.rds')
model_lda_22 <- readRDS('./Models/model_lda_22.rds')
```
...
TopicInDoc <- function(mod,docNum, topicLabels){
    TopicDocDist <- as.data.frame(mod@gamma)
    colnames(TopicDocDist) <- topicLabels
    #barplot(unlist(TopicDocDist[docn,]))
    
    #Print top 3 topic names
    temp <- t(TopicDocDist[docNum,])
...
writeLines(df[docnum,])
TopicInDoc(model_lda_21,docnum,Topic2Names)
```
